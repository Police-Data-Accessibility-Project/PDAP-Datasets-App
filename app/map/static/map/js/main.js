...
var state_geojson = L.geoJSON(states, {
    ...
    state_geojson.on('click', function(e) { 
        ...
        var settings = {
            "url": '/intake/ajax/get_agencies/' + state_abbr + '/',
            "method": "GET",
            "timeout": 0
        };
        ...
        $.getJSON(settings).done(function (response) {
            ...
            result.forEach(agency => {
                // make sure to only display ones with lats and lngs
                if (agency.fields.lat && agency.fields.lng)
                L.marker([agency.fields.lat, agency.fields.lng], {icon: police_station}).addTo(map).bindPopup("<p>"+agency.fields.name+"<p>");
            });
        });
